...
#SBATCH --mem=32G
#SBATCH --output=outputs/test_primary_key_limited.out
...
OUTPUT_DIR="data/output/test_primary_key_limited"
...
# Extract first 10 lines from original prompts (using 10 instead of 100 to limit API calls)
echo "Extracting first 10 lines from Hindi prompts..."
head -n 100 $HINDI_PROMPTS > $OUTPUT_DIR/hindi_10.txt

echo "Extracting first 10 lines from English prompts..."
head -n 100 $ENGLISH_PROMPTS > $OUTPUT_DIR/english_10.txt
...
python src/add_primary_key.py \
    --hindi $OUTPUT_DIR/hindi_10.txt \
    --english $OUTPUT_DIR/english_10.txt \
...
# Step 6: Generate LLaMA responses for Hindi prompts (sample 5 for speed)
echo "Step 6: Generating LLaMA responses for Hindi prompts..."
# First, extract 5 random filtered prompts for quick testing
python -c "
import pandas as pd
df = pd.read_csv('$OUTPUT_DIR/hindi/filtered_output.csv')
if len(df) > 5:
    df = df.sample(5, random_state=42)
df.to_csv('$OUTPUT_DIR/hindi/sample_5_filtered.csv', index=False)
print(f'Sampled {len(df)} prompts for quick testing')
"

python src/generate_model_responses_with_id.py \
    --input $OUTPUT_DIR/hindi/sample_5_filtered.csv \
    --model "meta-llama/Meta-Llama-3-8B-Instruct" \
    --output $OUTPUT_DIR/model_responses/llama_hindi_responses.csv \
    --max_tokens 100 \
    --temperature 0.7

# Step 7: Generate LLaMA responses for English prompts (use all since there are likely few)
...
# Step 8: Generate Aya responses for Hindi prompts (using same 5 samples)
...
# Step 9: Generate Aya responses for English prompts (all filtered)
...
# Step 10: Analyze toxicity of Hindi prompts (using 5 samples)
...
# Step 11: Analyze toxicity of LLaMA responses for Hindi prompts
echo "Step 11: Analyzing toxicity of LLaMA responses for Hindi prompts..."
python src/analyze_toxicity_with_id.py \
...
# Step 12: Analyze toxicity of English prompts (all filtered)
echo "Step 12: Analyzing toxicity of English prompts..."
python src/analyze_toxicity_with_id.py \
    --input $OUTPUT_DIR/english/filtered_output.csv \
...
# Step 17: Compare toxicity for English prompts and LLaMA responses
echo "Step 17: Comparing toxicity for English prompts and LLaMA responses..."
python src/compare_toxicity_with_id.py \
...
# Step 18: Create verification report to check primary key consistency
echo "Step 18: Creating verification report for primary key tracking..."
python -c "
import pandas as pd
import os
import glob
...
files_to_check = [
    ...
    # Sample filtered
    (os.path.join(base_dir, 'hindi', 'sample_5_filtered.csv'), 'Hindi sample 5 filtered'),
...
if os.path.exists(os.path.join(base_dir, 'hindi', 'sample_5_filtered.csv')) and os.path.exists(os.path.join(base_dir, 'model_responses', 'llama_hindi_responses.csv')):
    hindi_filtered = pd.read_csv(os.path.join(base_dir, 'hindi', 'sample_5_filtered.csv'))
...
if os.path.exists(os.path.join(base_dir, 'english', 'filtered_output.csv')) and os.path.exists(os.path.join(base_dir, 'model_responses', 'llama_english_responses.csv')):
    english_filtered = pd.read_csv(os.path.join(base_dir, 'english', 'filtered_output.csv'))
...
# Check toxicity analysis consistency
...
echo "-----------------------------"
echo "Hindi prompts: $(wc -l < $OUTPUT_DIR/hindi/sample_5_filtered.csv) requests (minus header)"
echo "English prompts: $(wc -l < $OUTPUT_DIR/english/filtered_output.csv) requests (minus header)"
echo "LLaMA Hindi responses: $(wc -l < $OUTPUT_DIR/model_responses/llama_hindi_responses.csv) requests (minus header)"
echo "LLaMA English responses: $(wc -l < $OUTPUT_DIR/model_responses/llama_english_responses.csv) requests (minus header)"
echo "Aya Hindi responses: $(wc -l < $OUTPUT_DIR/model_responses/aya_hindi_responses.csv) requests (minus header)"
echo "Aya English responses: $(wc -l < $OUTPUT_DIR/model_responses/aya_english_responses.csv) requests (minus header)"
echo "-----------------------------"
